...
# LATENT_DIM=32,EPOCHS=100,PERM=4,SEED=1701
...
df_train <- df_train[df_train$X45 %in%  c(10,14,18,24,30),]#10:35, ]

# Filter df_test for letters only
df_test <- df_test[df_test$X41 %in%  c(10,14,18,24,30),]#10:35, ] 10:35, ]
...
encoded_imgs <- predict(encoder, test_images)
...
# Find indexes of 'A' (label 10) in the test set
#indexes_A <- which(labels_test == 10)

# Select original and reconstructed images for 'A'
#original_images_A <- test_images#[indexes_A, ]
#reconstructed_images_A <- decoded_imgs#[indexes_A, ]

# Set up plotting area with reduced margins
...
vimpEncodedtt <- function(encoder, testData, num_permutations = 4, metric = "MSE") {
  ...
  #varimp <- matrix(0, nrow = 408, ncol = 8) # Initialize with 0 for summing
  # varimp <- matrix(0, nrow = 824, ncol = 8) # Initialize with 0 for summing
  ...
  for (j in 1:edim) {
    ...
    #feature_importance_matrix <- matrix(varimp[, j], nrow = edim, ncol = edim)
...
vimpDecodedNewtt <- function(encoder,
  ...
  if(letter){
    num_to_letter <- setNames(as.character(LETTERS[digits-9]), digits)
    #Transform the Digit column
    importance_df <- importance_df |>
      dplyr::mutate(Digit = num_to_letter[as.character(Digit)])
  }
  # if(encoderModel$type == "emnist"){
  #   num_to_letter <- setNames(as.character(LETTERS), edim:35)
  #
  #   # Transform the Digit column
  #   importance_df <- importance_df |>
  #     dplyr::mutate(Digit = num_to_letter[as.character(Digit)])
...
shinyDecodedLm_test <- function(decodedVimp, lmSummary, encodedVimp) {
  ...
  ui <- fluidPage(
    ...
    fluidRow(
      column(
        4,
        selectInput("digit", "Select Class:", choices = LETTERS, selected = "A")
      )
    ),
    ...
    fluidRow(
      column(
        4,
        numericInput("topX", "Select Top Dimensions:", value = 5)
      ),
      column(
        4,
        checkboxInput("filterZeros", "Filter Zero Importances", value = TRUE)
      ),
      column(
        4,
...
set.seed(1701)
ae_vimp_enc <- vimpEncodedtt(encoder = encoder, testData = test_images, num_permutations = 4)
...
ae_vimp_dec <- vimpDecodedNewtt(encoder = encoder,
                                ...
                                y_test = labels_test,
                                autoencoder = autoencoder,
                                digits =  c(10,14,18,24,30), #c(10:35),
                                num_permutations = 4,
                                letter = T)
# LM
edim = 32
...
plotDecodedtt(data = ae_vimp_dec)
...
#autoImp::plotDifference(ae_vimp_dec, 3,8)
