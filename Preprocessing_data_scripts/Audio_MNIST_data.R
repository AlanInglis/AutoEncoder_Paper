...
autoencoder %>% fit(
  ...
  epochs = 100,
  batch_size = 256,
  shuffle = F, # set to F for reproducible
...
for (i in 1:10) {
  image(1:32, 1:32, flip_matrix_vertically(original_reshaped[i,,]),
        col = viridis::viridis(255), main = paste("Input", i), axes = FALSE)
}

# Plot reconstructed images on the bottom row
for (i in 1:10) {
  image(1:32, 1:32, flip_matrix_vertically(reconstructed_reshaped[i,,]),
        col = viridis::viridis(255), main = paste("Output", i), axes = FALSE)
}
...
# SAVE OBJECTS ------------------------------------------------------------
save_model_tf(autoencoder, "autoencoder_model.keras")
save_model_tf(encoder, "encoder_model.keras")
saveRDS(decoded_imgs, "decoded_images.rds")
